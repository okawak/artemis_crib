...
using art::TNormalTreeProjection;

ClassImp(TNormalTreeProjection)
...
TNormalTreeProjection::TNormalTreeProjection()
...
TNormalTreeProjection::TNormalTreeProjection(const TNormalTreeProjection& rhs)
{
   // not implemented
   MayNotUse("TNormalTreeProjection");
}

TNormalTreeProjection& TNormalTreeProjection::operator=(const TNormalTreeProjection& rhs)
{
   // not implemented
   MayNotUse("operator=");
   if (this != &rhs) {
...
Bool_t TNormalTreeProjection::LoadYAMLNode(const YAML::Node &node)
{
   // local variables
      ...
      for (YAML::const_iterator itcont = contents.begin(); itcont != contents.end(); itcont++) {
         ...
         if (const YAML::Node clone = (*itcont)[kNodeKeyClone]) {
            if (TObject *obj = group->FindObject(clone.as<std::string>().c_str())) {
               // cloning the existing group;
               hproj = dynamic_cast<TNormalAttTreeProj*>(obj->Clone(hname));
            ...
            if (axisnode[2]) {
               ...
               hproj = new TH3FNormalTreeProj(hname,htitle,hcut);
               hproj->SetAxisAsync(hasync.Atoi());
               nDim = 3;
            } else if (axisnode[1]) {
               if (!axisnode[0]) {
               ...
               hproj = new TH2FNormalTreeProj(hname,htitle,hcut);
               hproj->SetAxisAsync(hasync.Atoi());
               nDim = 2;
            } else if (axisnode[0]) {
               // 1D histogram
               hproj = new TH1FNormalTreeProj(hname,htitle,hcut);
               hproj->SetAxisAsync(hasync.Atoi());
               nDim = 1;
            } else {
...
Bool_t TNormalTreeProjection::Sync(TTree *tree)
{
...
Bool_t TNormalTreeProjection::Sync(TTreeProjGroup *group, TTree *tree, TCut cut)
{
