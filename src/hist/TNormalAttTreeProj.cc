...
using art::TNormalAttTreeProj;

ClassImp(TNormalAttTreeProj)
namespace {
   template <typename T>
   struct delete_ptr {
      void operator() (T * p) {
...
TNormalAttTreeProj::TNormalAttTreeProj()
   : fH(NULL),fAxisAsync(kFALSE), fNeedSync(kTRUE)
{
}
TNormalAttTreeProj::TNormalAttTreeProj(TH1* h, const char *cut)
...
TNormalAttTreeProj::TNormalAttTreeProj(const TNormalAttTreeProj& rhs)
{
}

TNormalAttTreeProj& TNormalAttTreeProj::operator=(const TNormalAttTreeProj& rhs)
{
   if (this != &rhs) {
...
void TNormalAttTreeProj::Copy(TAttCut &newattin) const
{
   TNormalAttTreeProj &newatt = (TNormalAttTreeProj&)newattin;
...
void TNormalAttTreeProj::FillTo(TH1* hist)
{
...
void TNormalAttTreeProj::FillTo(TH2* hist)
{
   if (!fAxisAsync) {
      TAxisTreeProj *xaxis = fAxes[0];
...
void TNormalAttTreeProj::FillTo(TH3* hist)
{
   if (!fAxisAsync) {
      TAxisTreeProj *xaxis = fAxes[0];
      TAxisTreeProj *yaxis = fAxes[1];
...
void TNormalAttTreeProj::SetAxisDef(UInt_t i, TAxisTreeProj* a) {
   TAxis *axis = NULL;
   switch(i) {
   case 0:
      // x-axis
