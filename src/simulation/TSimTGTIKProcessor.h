...
#ifndef _TSIMTGTIKPROCESSOR_H_
#define _TSIMTGTIKPROCESSOR_H_

#include <TProcessor.h>

namespace art {
   class TSimTGTIKProcessor;
}

class TClonesArray;

class art::TSimTGTIKProcessor : public TProcessor {
public:
   // Default constructor
   TSimTGTIKProcessor();
   virtual ~TSimTGTIKProcessor();
...
protected:
   ...
   Float_t beam_width = 1.53f; // MeV

private:
   // Copy constructor (prohibited)
   TSimTGTIKProcessor(const TSimTGTIKProcessor& rhs);
   // Assignment operator (prohibited)
   TSimTGTIKProcessor& operator=(const TSimTGTIKProcessor& rhs);

   ClassDef(TSimTGTIKProcessor,1) // processor for calibration of timing and charge data
};

#endif // _TSIMTGTIKPROCESSOR_H_
